...
/* 2024/10/11 test dxf's Mytimer func */ 
...
/* 2024/10/11 test dxf's Mytimer func end */ 

/* 2024/10/17 test BMT code */

void test_BMT(){
    
    // dev::mptstate::MPTState::state_erasure = new ec::Eurasure();
    auto state_erasure = new ec::Eurasure();
    auto data_list = unordered_map<h256, uint>();
    for(int i=0; i<4; i++){
        auto tmp = u160(rand() % 1000000);
        std::cout<< "id:" <<tmp<<std::endl;
        auto h = sha3(dev::toString(tmp));
        data_list[h] = uint(i+1);
    }
...
void test_encodingStates(dev::mptstate::MPTState& mptState){
    /* 2024/10/23 状态编码*/
    auto mut_map = mptState.getState().db().get();
    std::cout << " The Map Get From OverlayDB is : \n";
    for(const auto& i: mut_map){
        std::cout << " H256: " << i.first << " String: "<< i.second << std::endl;
    }
...
void test_EC(){
    ...
    auto ec_ptr = new ec::Eurasure();
    std::string a = "howhowhow";
    std::string b = "areare";
    std::string c = "you";
    ...
    v.push_back(b);
    v.push_back(c);
    auto mpt = _MerkleTree(v);
    mpt.printTree();

    std::string abc = "howareyou";
    auto max_len = a.size();
    ec_ptr->setK(3);
    ec_ptr->setM(1);
    uint8_t* data = new uint8_t[max_len * 4 * sizeof(uint8_t)]();
    memset(data, 0, max_len * 4 * sizeof(uint8_t));
    int64_t count = 0;
    //将数据预处理
    for(auto t: v){
        memcpy(data + count, t.c_str(), t.size());
        count += max_len;
        cout<<t<<endl;
    }
    auto tmp = ec_ptr->encode(std::make_pair(data, max_len));
    auto lengh = tmp.second;
    auto ptrs = tmp.first;
    ...
    for (int count = 0; count < 4; count++)
    {
        ...
        std::cout << "Encode::Chunks[" << count << "] = " << value << std::endl;    
    }

    for(int i = 0; i<4; i++){
        for(int j = 0; j < max_len; j++){
            memset(ptrs[i], 0, 1 * 1 * sizeof(uint8_t));
            memset(ptrs[i] + 4, 0, (max_len - 4) * sizeof(uint8_t));
        }
    }
    memset(ptrs[0], 0, max_len * 1 * sizeof(uint8_t));
    for (int count = 0; count < 4; count++)
    {
        // 前面一个是该数据段开始的指针，后面int类型是截取的字符数量
        string value((const char*)ptrs[count], lengh);
    ...
    erasure_bool* present = new erasure_bool[4];
    memset(present, true, 4 * sizeof(erasure_bool));
    present[0] = false;
    erasure_encoder_parameters params = {4, 3, max_len};
    erasure_encoder* encoder = erasure_create_encoder(&params, ec_ptr->getecmode());
...
int main(int argc, char* argv[])
{
...

    {
        ...
        for (int i = 1; i <= _block_num; i++) {
            ...
            // auto _bal = mptState.balance(u160(289383));
        ...
        for(int i=1; i<=data_map.size(); i++){
            // int _dist = data_map[i].size()/ 100; 
            int _dist = 1; // 测试子切分专用
            auto t6 = std::chrono::steady_clock::now();
            for(int j=0; j < 100; j = j + _dist){
                // cout << " [Recovering Data] :" << data <<endl;
                // auto ver =  mptState.versionManager.m_currentVersion + mptState.versionManager.m_nodeVersions[_data];
                auto visit_index = rand() % data_map[i].size();
                auto &data = data_map[i][visit_index];
                mediator.recoverStateInParallel(data, 0, i);
                _o++;
                // cout<< " end one round" << endl;
                // _o = _o + data_map[i].size()/10;
            }
            auto t7 = std::chrono::steady_clock::now();
            auto read_time = std::chrono::duration_cast<std::chrono::microseconds>(t7 - t6).count() / 1000.0;
            cout <<  "Encoding time for some malicious nodes (AVG) " << read_time / (double)_o << endl;
