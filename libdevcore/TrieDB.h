...
//}
    ...
    /// Used for debugging, scans the whole trie.
    // 2024/10/23 重载 descendKey 函数，将收集MPT相关信息以进行更高效的状态分片
    void descendKey(
        h256 const& _k, h256Hash& _keyMask, bool _wasExt, std::vector<h256>& _v, int _indent = 0) const
    {
        if(_keyMask.find(_k) == _keyMask.end()){
            return;
        }
        _keyMask.erase(_k);
        if (_k == m_root && _k == EmptyTrie)  // root allowed to be empty
        ...
        std::string const s = node(_k);
        RLP r = RLP(s);
        descendList(r, _keyMask, _wasExt, _v, _indent);  // if not, it must be a list
    }
    ...
    /// Used for debugging, scans the whole trie.
    // 2024/10/23 重载 descendEntry 函数，将收集MPT相关信息以进行更高效的状态分片
    void descendEntry(
        RLP const& _r, h256Hash& _keyMask, bool _wasExt, std::vector<h256>& _v, int _indent) const
    {
        if (_r.isData() && _r.size() == 32)
            descendKey(_r.toHash<h256>(), _keyMask, _wasExt, _v, _indent);
        else if (_r.isList())
            descendList(_r, _keyMask, _wasExt, _v, _indent);
        else
            BOOST_THROW_EXCEPTION(InvalidTrie());
    }
    ...
    /// Used for debugging, scans the whole trie.
    // 2024/10/23 重载 descendList 函数，将收集MPT相关信息以进行更高效的状态分片
    void descendList(
        RLP const& _r, h256Hash& _keyMask, bool _wasExt, std::vector<h256>& _v, int _indent) const
    {
        if (_r.isList() && _r.itemCount() == 2 && (!_wasExt))
        {
            ...
            //     (*_out) << std::string(_indent * 2, ' ') << (_wasExt ? "!2 " : "2  ")
            //             << sha3(_r.data()) << ": " << _r << "\n";
            _v.push_back(sha3(_r.data()));
            if (!isLeaf(_r))  // don't go down leaves
                descendEntry(_r[1], _keyMask, true, _v, _indent + 1);
            else{
                // h256 leaf_sign;
                // _v.push_back(leaf_sign);
            }
        }
        else if (_r.isList() && _r.itemCount() == 17)
        {
            ...
            for (unsigned i = 0; i < 16; ++i)
                if (!_r[i].isEmpty())  // 16 branches are allowed to be empty
                    descendEntry(_r[i], _keyMask, false, _v, _indent + 1);
        }
        else
            BOOST_THROW_EXCEPTION(InvalidTrie());
    }
    ...
    /// Used for debugging, scans the whole trie.
    // 2024/10/23 重载 descendList 函数，将收集MPT相关信息以进行更高效的状态分片
    h256Hash leftOvers(std::vector<h256>& _v) const
    {
        h256Hash k = m_db->keys();
        for(auto& _k: k){
            // std::cout<<"--->key:"<<_k<<std::endl;
        }
        descendKey(m_root, k, false, _v);
        return k;
    }
...
private:
    ...
    void forceKillNode(h256 const& _h) { 
        // std::cout<< "_____Kill "<< _h << std::endl; 
        m_db->kill(_h); 
    }
...
public:
    ...
    {
        // std::cout<<"KeyType at:"<< _k << "#" << "" << sizeof(KeyType) << std::endl;
...
public:
    ...
    std::string at(bytesConstRef _key) const { 
        // std::cout<<"sha3 : "<< sha3(_key) <<std::endl; 
        return Super::at(sha3(_key)); 
    }
...
{
    // std::cout<<"TrieDB insert"<<std::endl;
...
std::string GenericTrieDB<DB>::atAux(RLP const& _here, NibbleSlice _key) const
{
    ...
    if (itemCount == 2)
    {
        ...
        //     std::cout <<  "到达叶子节点，其值为：" << RLP(_here[1].toString()) << std::endl;
        // else if (_key.contains(k) && !isLeaf(_here))
        //     // not yet at leaf and it might yet be us. onwards...
        //     std::cout <<  "到达扩展节点，其下个节点的hash为:" << _here[1].toHash<h256>() << std::endl;
        // else
        //     // not us.
        //     std::cout << "查询值不存在" << std::string() << std::endl;
    }
    else
    {
        // if (_key.size() == 0)
