...
Account* State::account(Address const& _addr)
{
    ...
    // Populate basic info.
    // std::cout<< "account func" << std::endl;
    ...
    RLP state(stateBack);
    // std::cout<<"stateBack---:"<< state <<std::endl;
    // std::cout<<"state[2].toHash<h256>():::::::::"<< state[2].toHash<h256>() <<std::endl;
...
u256 State::storage(Address const& _id, u256 const& _key)
{
    ...
    if (Account* a = account(_id))
    {
        ...
        std::cout<<"Not in the storage cache - go to the DB."<<std::endl;
        x_db.lock();
...
AddressHash dev::mptstate::commit(AccountMap const& _cache, SecureTrieDB<Address, DB>& _state)
{
    ...
    // 2024/10/14 这里的i主要为<Address, Account>  
    // 该func主要将 _cache 中的数据以 bytes 的形式塞入 m_state 中
    ...
    for (auto const& i : _cache)
        if (i.second.isDirty())
        {
            ...
            else
            {
                ...
                // std::cout << "插入的Address为:" << i.first << std::endl;
